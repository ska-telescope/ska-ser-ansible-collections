...
NAMESPACES=$(kubectl get namespaces --field-selector status.phase=Active -o json | jq --arg regex "$SHARED_NAMESPACE_REGEX" -r '.items[] | select ((.metadata.annotations.SKA_SHARED_ENV_TTL | . == null or fromdateiso8601) < now) |  select(.metadata.name | test($regex)).metadata.name')
for NS in $NAMESPACES; do
	kubectl delete namespace $NS
done
