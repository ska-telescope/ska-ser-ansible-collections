...
GITLAB_RUNNER_NODE_SELECTOR ?= node-role.skatelescope.org/ci-worker
...
vars:
	...
	@echo "GITLAB_RUNNER_NAMESPACE=$(GITLAB_RUNNER_NAMESPACE)"
	@echo "GITLAB_RUNNER_RELEASE=$(GITLAB_RUNNER_RELEASE)"
	@echo "GITLAB_RUNNER_MINIO_RELEASE=$(GITLAB_RUNNER_MINIO_RELEASE)"
	@echo "GITLAB_RUNNER_RELEASE=$(GITLAB_RUNNER_RELEASE)"
	@echo "GITLAB_RUNNER_NODE_SELECTOR=$(GITLAB_RUNNER_NODE_SELECTOR)"
...
envsubst:
	@GITLAB_RUNNER_RELEASE=$(GITLAB_RUNNER_RELEASE) \
	 GITLAB_RUNNER_METRICS_PORT=$(GITLAB_RUNNER_METRICS_PORT) \
	 envsubst < $(PLAYBOOKS_DIR)/../roles/manifestgen/files/runner-metrics.yaml.in >$(PLAYBOOKS_DIR)/../roles/manifestgen/files/runner-metrics.yaml

tidy:  ## Clean up patch files
	@rm -rf ./runner_kustomize/cache-secret.yaml \
	./playbooks/files/minio_tenant.yml

manifestgen_runner: tidy envsubst  ## Deploy runners
	ansible-playbook $(PLAYBOOKS_DIR)/generate_manifest_runner.yml \
	-i $(INVENTORY) $(ANSIBLE_PLAYBOOK_ARGUMENTS) $(ANSIBLE_EXTRA_VARS) \
	--extra-vars "target_hosts=$(PLAYBOOKS_HOSTS)" \
	$(GITLAB_RUNNER_TAG_LIST_ARG) \
	-e "gitlab_runner_manifestgen_gitlab_runner_namespace=$(GITLAB_RUNNER_NAMESPACE)" \
	-e "gitlab_runner_manifestgen_gitlab_release_name='$(GITLAB_RUNNER_RELEASE)'" \
	-e "gitlab_runner_manifestgen_gitlab_node_selector='$(GITLAB_RUNNER_NODE_SELECTOR)'" \
	-e "gitlab_runner_manifestgen_gitlab_s3_bucket_name=$(GITLAB_RUNNER_MINIO_BUCKET_NAME)" \
	-e "gitlab_runner_manifestgen_gitlab_s3_bucket_location=$(GITLAB_RUNNER_MINIO_BUCKET_LOCATION)" \
...
deploy_minio: tidy envsubst  ## Deploy Minio
	...
	$(GITLAB_RUNNER_TAG_LIST_ARG) \
	--extra-vars "target_hosts=$(PLAYBOOKS_HOSTS)" \
	--extra-vars="gitlab_runner_minio_namespace=$(GITLAB_RUNNER_NAMESPACE) minio_release_name='$(GITLAB_RUNNER_MINIO_RELEASE)'" \
...
runner_logs: ## Show runner logs
	@for i in `kubectl -n $(GITLAB_RUNNER_NAMESPACE) get pods -l release=$(GITLAB_RUNNER_RELEASE) -o=name`; \
	do \
	echo "---------------------------------------------------"; \
	echo "Logs for $${i}"; \
	echo kubectl -n $(GITLAB_RUNNER_NAMESPACE) logs $${i}; \
	echo kubectl -n $(GITLAB_RUNNER_NAMESPACE) get $${i} -o jsonpath="{.spec.initContainers[*].name}"; \
	echo "---------------------------------------------------"; \
	for j in `kubectl -n $(GITLAB_RUNNER_NAMESPACE) get $${i} -o jsonpath="{.spec.initContainers[*].name}"`; do \
	RES=`kubectl -n $(GITLAB_RUNNER_NAMESPACE) logs $${i} -c $${j} 2>/dev/null`; \
	echo "initContainer: $${j}"; echo "$${RES}"; \
	echo "---------------------------------------------------";\
	done; \
	echo "Main Pod logs for $${i}"; \
	echo "---------------------------------------------------"; \
	for j in `kubectl -n $(GITLAB_RUNNER_NAMESPACE) get $${i} -o jsonpath="{.spec.containers[*].name}"`; do \
	RES=`kubectl -n $(GITLAB_RUNNER_NAMESPACE) logs $${i} -c $${j} 2>/dev/null`; \
	echo "Container: $${j}"; echo "$${RES}"; \
	echo "---------------------------------------------------";\
	echo "config.toml for: $${j}"; \
	rm -f /tmp/config.toml; \
	kubectl -n $(GITLAB_RUNNER_NAMESPACE) cp $$(echo "$${i}" | cut -d/ -f 2):/home/gitlab-runner/.gitlab-runner /tmp/; \
	echo "---------------------------------------------------";\
	cat /tmp/config.toml; \
	done; \
	done
...
minio_logs: ## Show minio logs
	@for i in `kubectl -n $(GITLAB_RUNNER_NAMESPACE) get pods -l app=minio -o=name`; \
	do \
	echo "---------------------------------------------------"; \
	echo "Logs for $${i}"; \
	echo kubectl -n $(GITLAB_RUNNER_NAMESPACE) logs $${i}; \
...
list_cache:  ## list the minio cache using in-cluster connection
	@kubectl -n $(GITLAB_RUNNER_NAMESPACE) run mc --rm -ti --image=minio/mc --restart=Never --command -- \
...
get_cache:  ## retrieve the minio cache
	sudo rm -rf $$(pwd)/tmp
	mkdir -p $$(pwd)/tmp
	kubectl -n $(GITLAB_RUNNER_NAMESPACE) port-forward service/minio 9001:80 &
	sleep 3
...
get_operator_jwt: ## get jwt token for operator console
	kubectl -n $(GITLAB_RUNNER_NAMESPACE) get secret $$(kubectl -n $(GITLAB_RUNNER_NAMESPACE) get serviceaccount console-sa -o jsonpath="{.secrets[0].name}") -o jsonpath="{.data.token}" | base64 --decode

forward_operator_console: ## port forward operator console on 9090
	kubectl -n $(GITLAB_RUNNER_NAMESPACE) port-forward svc/console 9090:9090

forward_tenant_console: ## port forward tenant console on 9091
	kubectl -n $(GITLAB_RUNNER_NAMESPACE) port-forward svc/minio-console 9091:9090
