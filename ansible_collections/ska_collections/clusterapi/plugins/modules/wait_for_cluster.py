...
short_description: Wait for a Cluster to become ready
...
description: Wait for a Cluster to become ready
...
options:
  ...
  phase:
    description:
      - Phase of Cluster machines - default "Running".
    required: false
    type: str
  kubectl_namespace:
    description:
      - Namespace of cluster - default "default".
    required: false
  state:
    description:
...
def get_machines(connector, name):
    ...
    for m in kubectl_yaml["items"]:
        mname = m.get("metadata").get("name")
        labels = m.get("metadata").get("labels")
        phase = m.get("status").get("phase")
        machines.append({"name": mname, "labels": labels, "phase": phase})
    connector._msg += ("\n MACHINES: {machines} \n ").format(machines=machines)
    return machines


def main():
    """Launch wait_for_cluster module plugin"""

    module_args = dict(
        name=dict(type="str", required=True),
        state=dict(type="str", required=False, default="present"),
        phase=dict(type="str", required=False, default="Running"),
    ...
    try:
        ...
        module.fail_json(
            msg="Failed to get_machines: [%s] %s" % (connector._msg, repr(e))
        )
